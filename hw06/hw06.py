...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, product_name, price, product_num = 0, funds = 0):
        self.product_name = product_name
        self.price = price
        self.product_num = product_num
        self.funds = funds
    
    def restock(self, product_num_restock):
        self.product_num += product_num_restock
        return 'Current {} stock: {}'.format(self.product_name, self.product_num)
    ...
    def vend(self):
        if self.product_num == 0:
            ...
            charge = self.funds - self.price
            self.funds = 0
            self.product_num -= 1
            return 'Here is your {0} and ${1} change.'.format(self.product_name, charge)
...
def is_bst(t):
    ...
    def bst_max(t):
        ...
        if t.is_leaf():
            ...
            return max(max([bst_max(branch) for branch in t.branches]), t.label)
    ...
    def bst_min(t):
        ...
        if t.is_leaf():
            ...
            return min(min([bst_min(branch) for branch in t.branches]), t.label)
    if t.is_leaf():
        return True
...
def path_yielder(t, value):
    ...
    if t.label is value:
        yield [t.label]
    if t.is_leaf() and t.label is not value:
        yield []
    for branch in t.branches:
        a = path_yielder(branch, value)
        for maybe_list in a:
            if value in maybe_list:
                yield [t.label] + maybe_list
...
def remove_all(link , value):
    ...
    while link is not Link.empty and link.rest is not Link.empty:
        while link.rest is not Link.empty and link.rest.first is value:
