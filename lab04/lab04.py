...
def reverse_iter(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    ans = [lst[i] for i in range(len(lst) - 1, -1, -1)]
    return ans
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_anoymous = make_city('a city', lat, lon)
    if distance(city_anoymous, city_a) < distance(city_anoymous, city_b):
...
def add_chars(w1, w2):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_chars_helper(w1, w2, i, j, ans):
        if i == len(w1) and j < len(w2):
            ...
            ans += w2[j]
            return add_chars_helper(w1, w2, i, j+1, ans)
        else:
            return add_chars_helper(w1, w2, i+1, j+1, ans)
    ans = ""
    return add_chars_helper(w1, w2, 0, 0, ans)
