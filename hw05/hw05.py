...
def make_withdraw(balance, password):
    ...
    def print_error():
        first = True
        s = "Too many incorrect attempts. Attempts: [\'"
        ...
        s += "]"
        return s
    def withdraw(amount, inter_password):
        nonlocal balance, error_list
        if len(error_list) == 3:
            return print_error()
        elif password == inter_password:
            if balance > amount:
                balance -= amount
                return balance
            else:
