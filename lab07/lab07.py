...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        return (lambda item, f: subseqs(s[1:]) + f(item, subseqs(s[1:])))(s[0], insert_into_all)
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(s[0], subseq_helper(s[1:], s[0])) + subseq_helper(s[1:], -1000)
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    current_score, current_all_score = 0, 0
    def student(string, score):
        nonlocal current_score, current_all_score
        current_score += score
        current_all_score += class_assignments[string]
        return current_score / current_all_score
    return student
