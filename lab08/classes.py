...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        sum = other_card.defense + other_card.attack
        for i in range(len(player.deck.cards)):
            player.deck.cards[i].defense = sum
            player.deck.cards[i].attack = sum
        copy = []
        for i in range(len(opponent.deck.cards)):
            if opponent.deck.cards[i].defense != other_card.defense or opponent.deck.cards[i].attack != other_card.attack:
                copy += [opponent.deck.cards[i]]
        opponent.deck.cards[:] = copy
